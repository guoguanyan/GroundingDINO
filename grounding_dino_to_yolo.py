...
if __name__ == "__main__":
    ...
    CONFIG_PATH_T = "groundingdino/config/GroundingDINO_SwinT_OGC.py"
    # CKPT_PATH_T   = "weights/groundingdino_swint_ogc.pth"
    
    # B模型
    CONFIG_PATH_B = "groundingdino/config/GroundingDINO_SwinB_cfg.py"
    # CKPT_PATH_B   = "weights/groundingdino_swinb_cogcoor.pth"


    # 自动从HF下载并返回本地缓存路径
    # 如果文件已存在则直接返回缓存路径，不会重复下载
    swinb_ckpt = hf_hub_download(
        repo_id="Stella-guo/GroundingDINO",
        filename="weights/groundingdino_swinb_cogcoor.pth",
        local_dir="./"  # 可选：指定本地缓存目录，不指定则用HF默认缓存
    )

    swint_ckpt = hf_hub_download(
        repo_id="Stella-guo/GroundingDINO",
        filename="weights/groundingdino_swint_ogc.pth",
        local_dir="./"
    )

    # 将下载后的真实本地路径赋值给原有变量
    # 注意：CONFIG_PATH 通常是本地代码文件，无需从HF下载
    CKPT_PATH_B = swinb_ckpt   # 对应 SwinB
    CKPT_PATH_T = swint_ckpt   # 对应 SwinT
    ...
    detector = GroundingDINOToYOLO(CONFIG_PATH_T, CKPT_PATH_T)
